...
func FromSwapSegmentJSON(segmentStr string) (segment *SwapSegment, err error) {
	...
	if segmentFormat.C_uA == "" || segmentFormat.C_fee == "" || segmentFormat.Pk_u == "" || segmentFormat.Pk_treasury == "" ||
		...
		return nil, errors.New("[FromSwapSegmentJSON] invalid params")
	}
	// verify params
	...
	if !isValid {
		...
		return nil, errors.New("[FromSwapSegmentJSON] invalid sk")
	}
