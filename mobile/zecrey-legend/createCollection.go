...
func ProveCreateCollection(seed string, segmentInfo string) (txInfo string, err error) {
	// parse segmentInfo
	sk, err := curve.GenerateEddsaPrivateKey(seed)
	if err != nil {
		return "", err
	}
	oTxInfo, err := legendTxTypes.ConstructCreateCollectionTxInfo(sk, segmentInfo)
	if err != nil {
		return "", err
	}
