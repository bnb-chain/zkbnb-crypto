...
import (
	...
	block2 "github.com/bnb-chain/zkbnb-crypto/circuit/bn254/block"
)

func TestExportSol(t *testing.T) {
	differentBlockSizes := []int{1, 10}
	for i := 0; i < len(differentBlockSizes); i++ {
		var circuit block2.BlockConstraints
		circuit.TxsCount = differentBlockSizes[i]
		circuit.Txs = make([]block2.TxConstraints, circuit.TxsCount)
		for i := 0; i < circuit.TxsCount; i++ {
			circuit.Txs[i] = block2.GetZeroTxConstraint()
		}
