...
func ComputeHashFromTransferNftTx(api API, tx TransferNftTxConstraints, nonce Variable, expiredAt Variable) (hashVal Variable) {
	return poseidon.Poseidon(api, ChainId, tx.FromAccountIndex, nonce, expiredAt, tx.GasFeeAssetId,
		tx.GasFeeAssetAmount, tx.ToAccountIndex,
		tx.NftIndex, tx.ToAccountNameHash, tx.CallDataHash)
}
