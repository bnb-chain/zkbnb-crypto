...
func (txInfo *WithdrawTxInfo) VerifySignature(pubKey string) error {
	...
	inner, err := abiEncoder.Pack("", messageTypeBytes32, txInfo.FromAccountIndex, txInfo.AssetId, ConvertStringHexToBytes16(txInfo.AssetAmount), ConvertStringHexToBytes20(txInfo.ToAddress), txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
