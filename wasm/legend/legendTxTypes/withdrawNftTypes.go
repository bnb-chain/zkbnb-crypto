...
func (txInfo *WithdrawNftTxInfo) VerifySignature(pubKey string) error {
	...
	abiEncoder, err := abiEth.JSON(strings.NewReader(abi.WithdrawNftABIJSON))
	if err != nil {
		return err
	}
	messageTypeBytes32 := abi.GetEIP712MessageTypeHashBytes32(abi.WithdrawNft)

	inner, err := abiEncoder.Pack("", messageTypeBytes32, txInfo.AccountIndex, txInfo.NftIndex, ConvertStringHexToBytes20(txInfo.ToAddress), txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
