...
func Enc(b *big.Int, r *big.Int, pk *P256) (*ElGamalEnc) {
	// pk^r
	CL := zp256.ScalarMul(pk, r)
	// g^r h^b
	CR := zp256.ScalarBaseMul(r)
	CR = zp256.Add(CR, zp256.ScalarHBaseMul(b))
	return &ElGamalEnc{CL: CL, CR: CR}
}
...
func Dec(enc *ElGamalEnc, sk *big.Int) (*big.Int) {
	...
	gExpr := zp256.ScalarMul(enc.CL, skInv)
	hExpb := zp256.Add(enc.CR, zp256.Neg(gExpr))
	for i := 0; i < MAX_VALUE; i++ {
		hi := zp256.ScalarHBaseMul(big.NewInt(int64(i)))
