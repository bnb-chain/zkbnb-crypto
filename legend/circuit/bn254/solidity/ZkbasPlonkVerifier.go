...
contract ZkbasPlonkVerifier is PlonkVerifier {
    ...
    function get_verification_key(uint16 block_size) internal pure returns(VerificationKey memory vk) {
        {{range $i, $vi := .Vks }}
		if (block_size == {{index $.Sizes $i}}) {
			vk.domain_size = {{$vi.Size}};
			vk.num_inputs = {{$vi.NbPublicVariables}};
			vk.omega = PairingsBn254.new_fr(uint256({{$vi.Generator.String}}));
			vk.selector_commitments[0] = PairingsBn254.new_g1(
				uint256({{$vi.Ql.X.String}}),
				uint256({{$vi.Ql.Y.String}})
			);
			vk.selector_commitments[1] = PairingsBn254.new_g1(
				uint256({{$vi.Qr.X.String}}),
				uint256({{$vi.Qr.Y.String}})
			);
			vk.selector_commitments[2] = PairingsBn254.new_g1(
				uint256({{$vi.Qo.X.String}}),
				uint256({{$vi.Qo.Y.String}})
			);
			vk.selector_commitments[3] = PairingsBn254.new_g1(
				uint256({{$vi.Qm.X.String}}),
				uint256({{$vi.Qm.Y.String}})
			);
			vk.selector_commitments[4] = PairingsBn254.new_g1(
				uint256({{$vi.Qk.X.String}}),
				uint256({{$vi.Qk.Y.String}})
			);
	
			vk.permutation_commitments[0] = PairingsBn254.new_g1(
				uint256({{(index $vi.S 0).X.String}}),
				uint256({{(index $vi.S 0).Y.String}})
			);
			vk.permutation_commitments[1] = PairingsBn254.new_g1(
				uint256({{(index $vi.S 1).X.String}}),
				uint256({{(index $vi.S 1).Y.String}})
			);
			vk.permutation_commitments[2] = PairingsBn254.new_g1(
				uint256({{(index $vi.S 2).X.String}}),
				uint256({{(index $vi.S 2).Y.String}})
			);
	
			vk.permutation_non_residues[0] = PairingsBn254.new_fr(
				uint256({{$vi.CosetShift.String}})
			);
			...
			vk.g2_x = PairingsBn254.new_g2(
				[uint256({{(index $vi.KZGSRS.G2 1).X.A1.String}}),
				uint256({{(index $vi.KZGSRS.G2 1).X.A0.String}})],
				[uint256({{(index $vi.KZGSRS.G2 1).Y.A1.String}}),
				uint256({{(index $vi.KZGSRS.G2 1).Y.A0.String}})]
			);
		}
		{{end}}
    }
